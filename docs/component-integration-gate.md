...
On one fresh Crabbox-managed Azure Windows desktop VM (`--keep=false`), build
or place the independently selected artifacts, then run:
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run-component-integration.ps1 `
  ...
  -EvidenceDirectory C:\ProgramData\crabbox\go-mapi-component-integration
```
...
files through the managed desktop session before stopping the VM. Missing
